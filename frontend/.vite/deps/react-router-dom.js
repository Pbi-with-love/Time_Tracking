...
function convertRoutesToDataRoutes(routes, mapRouteProperties2, parentPath = [], manifest = {}, allowInPlaceMutations = false) {
  return routes.map((route, index) => {
    ...
    let id = typeof route._id === "string" ? route._id : treePath.join("-");
    invariant(
...
function convertRouteMatchToUiMatch(match, loaderData) {
  ...
  return {
    ...
    pathname,
    params,
    data: loaderData[route._id],
    loaderData: loaderData[route._id],
    handle: route.handle
  };
}
...
function createRouter(init) {
  ...
  function updateState(newState, opts = {}) {
    if (newState.matches) {
      newState.matches = newState.matches.map((m) => {
        let route = manifest[m.route._id];
  ...
  async function startNavigation(historyAction, location2, opts) {
    ...
    if (opts && opts.pendingError) {
      pendingActionResult = [
        findNearestBoundary(matches).route._id,
  ...
  async function handleAction(request, location2, submission, matches, scopedContext, isFogOfWar, initialHydration, opts = {}) {
    ...
    if (isFogOfWar) {
      ...
      if (discoverResult.type === "aborted") {
        ...
        let boundaryId = findNearestBoundary(discoverResult.partialMatches).route._id;
        return {
    ...
    if (!actionMatch.route.action && !actionMatch.route.lazy) {
      ...
      result = results[actionMatch.route._id];
      if (!result) {
        for (let match of matches) {
          if (results[match.route._id]) {
            result = results[match.route._id];
            break;
          }
        }
      }
    ...
    if (isErrorResult(result)) {
      let boundaryMatch = findNearestBoundary(matches, actionMatch.route._id);
      ...
      return {
        matches,
        pendingActionResult: [
          boundaryMatch.route._id,
          result,
          actionMatch.route._id
        ]
      };
    }
    return {
      matches,
      pendingActionResult: [actionMatch.route._id, result]
    };
  }
      ...
      if (discoverResult.type === "aborted") {
        ...
        if (discoverResult.partialMatches.length === 0) {
          ...
          return {
            ...
            loaderData: {},
            errors: {
              [route._id]: discoverResult.error
            }
          };
        }
        let boundaryId = findNearestBoundary(discoverResult.partialMatches).route._id;
  ...
  async function handleFetcherAction(key, routeId, path, requestMatches, scopedContext, isFogOfWar, flushSync3, preventScrollReset, submission) {
    ...
    let actionResult = actionResults[match.route._id];
  ...
  async function handleFetcherLoader(key, routeId, path, matches, scopedContext, isFogOfWar, flushSync3, preventScrollReset, submission) {
    ...
    let result = results[match.route._id];
    if (fetchControllers.get(key) === abortController) {
      fetchControllers.delete(key);
    }
    if (fetchRequest.signal.aborted) {
    ...
    let fetcherResultsPromise = Promise.all(
      fetchersToLoad.map(async (f) => {
        if (f.matches && f.match && f.request && f.controller) {
          ...
          let result = results[f.match.route._id];
          return { [f.key]: result };
        } else {
  ...
  async function discoverRoutes(matches, pathname, signal, fetcherKey) {
    ...
    while (true) {
      ...
      if (!newPartialMatches || partialMatches.length === newPartialMatches.length && partialMatches.every(
        (m, i) => m.route._id === newPartialMatches[i].route._id
      )) {
...
function createStaticHandler(routes, opts) {
          ...
          async (error, routeId) => {
            ...
            if (renderedStaticContext) {
              ...
              let staticContext = getStaticContextFromError(
                ...
                error,
                skipLoaderErrorBubbling ? routeId : findNearestBoundary(matches, routeId).route._id
              );
              ...
              let boundaryRouteId = skipLoaderErrorBubbling ? routeId : findNearestBoundary(
                matches,
                matches.find(
                  (m) => m.route._id === routeId || m.route.loader
                )?.route._id || routeId
              ).route._id;
              let staticContext = {
                matches,
  ...
  async function queryRoute(request, {
    ...
    let match = routeId ? matches.find((m) => m.route._id === routeId) : getTargetMatch(matches, location2);
    if (routeId && !match) {
  ...
  async function submit(request, matches, actionMatch, requestContext, dataStrategy, skipLoaderErrorBubbling, isRouteRequest, filterMatchesToLoad, skipRevalidation) {
    ...
    if (isRouteRequest) {
      ...
      return {
        matches: [actionMatch],
        loaderData: {},
        actionData: { [actionMatch.route._id]: result.data },
        errors: null,
    ...
    if (skipRevalidation) {
      if (isErrorResult(result)) {
        let boundaryMatch = skipLoaderErrorBubbling ? actionMatch : findNearestBoundary(matches, actionMatch.route._id);
        return {
          statusCode: isRouteErrorResponse(result.error) ? result.error.status : result.statusCode != null ? result.statusCode : 500,
          actionData: null,
          actionHeaders: {
            ...result.headers ? { [actionMatch.route._id]: result.headers } : {}
          },
          matches,
          loaderData: {},
          errors: {
            [boundaryMatch.route._id]: result.error
          },
          loaderHeaders: {}
        };
      } else {
        return {
          actionData: {
            [actionMatch.route._id]: result.data
          },
          actionHeaders: result.headers ? { [actionMatch.route._id]: result.headers } : {},
          matches,
          loaderData: {},
          errors: null,
    ...
    if (isErrorResult(result)) {
      ...
      let handlerContext2 = await loadRouteData(
        ...
        null,
        filterMatchesToLoad,
        [boundaryMatch.route._id, result]
      );
      return {
        ...handlerContext2,
        statusCode: isRouteErrorResponse(result.error) ? result.error.status : result.statusCode != null ? result.statusCode : 500,
  ...
  async function loadRouteData(request, matches, requestContext, dataStrategy, skipLoaderErrorBubbling, routeMatch, filterMatchesToLoad, pendingActionResult) {
    ...
    if (routeMatch) {
      ...
      let maxIdx = pendingActionResult && isErrorResult(pendingActionResult[1]) ? (
        ...
        matches.findIndex((m) => m.route._id === pendingActionResult[0]) - 1
      ) : void 0;
  ...
  async function callDataStrategy(request, matches, isRouteRequest, requestContext, dataStrategy) {
    ...
    await Promise.all(
      matches.map(async (match) => {
        ...
        dataResults[match.route._id] = await convertDataStrategyResultToDataResult(result);
      })
    );
    return dataResults;
  }
...
function getStaticContextFromError(routes, handlerContext, error, boundaryId) {
  let errorBoundaryId = boundaryId || handlerContext._deepestRenderedBoundaryId || routes[0]._id;
  return {
    ...handlerContext,
    statusCode: isRouteErrorResponse(error) ? error.status : 500,
    errors: {
...
function normalizeTo(location2, matches, basename, to, fromRouteId, relative) {
  ...
  if (fromRouteId) {
    ...
    for (let match of matches) {
      ...
      if (match.route._id === fromRouteId) {
...
function getMatchesToLoad(request, scopedContext, mapRouteProperties2, manifest, history, state, matches, submission, location2, lazyRoutePropertiesToSkip, initialHydration, isRevalidationRequired, cancelledFetcherLoads, fetchersQueuedForDeletion, fetchLoadMatches, fetchRedirectIds, routesToUse, basename, hasPatchRoutesOnNavigation, pendingActionResult) {
  ...
  if (initialHydration && state.errors) {
    ...
    maxIdx = matches.findIndex((m) => m.route._id === boundaryId);
  } else if (pendingActionResult && isErrorResult(pendingActionResult[1])) {
    let boundaryId = pendingActionResult[0];
    maxIdx = matches.findIndex((m) => m.route._id === boundaryId) - 1;
  }
  ...
  fetchLoadMatches.forEach((f, key) => {
    if (initialHydration || !matches.some((m) => m.route._id === f.routeId) || fetchersQueuedForDeletion.has(key)) {
      return;
    }
    let fetcher = state.fetchers.get(key);
...
function shouldLoadRouteOnHydration(route, loaderData, errors) {
  ...
  let hasData = loaderData != null && route._id in loaderData;
  let hasError = errors != null && errors[route._id] !== void 0;
  if (!hasData && hasError) {
    return false;
  }
...
function isNewLoader(currentLoaderData, currentMatch, match) {
  let isNew = (
    ...
    match.route._id !== currentMatch.route._id
  );
  let isMissingData = !currentLoaderData.hasOwnProperty(match.route._id);
...
function isSameRoute(newRoute, existingRoute) {
  if ("id" in newRoute && "id" in existingRoute && newRoute._id === existingRoute._id) {
    return true;
  }
...
var loadLazyRouteProperty = ({
  ...
  let routeToUpdate = manifest[route._id];
  ...
  let propertyPromise = (async () => {
    ...
    if (isUnsupported) {
      ...
      warning(
        false,
        `Route "${routeToUpdate._id}" has a static property "${key}" defined. The lazy property will be ignored.`
      );
    } else {
    ...
    let lazyRoutePromise2 = (async () => {
      ...
      for (let lazyRouteProperty in lazyRoute) {
        ...
        if (isUnsupported) {
          ...
          warning(
            !isStaticallyDefined,
            `Route "${routeToUpdate._id}" has a static property "${lazyRouteProperty}" defined but its lazy function is also returning a value for this property. The lazy route property "${lazyRouteProperty}" will be ignored.`
          );
        } else {
...
async function defaultDataStrategy(args) {
  ...
  results.forEach((result, i) => {
    keyedResults[matchesToLoad[i].route._id] = result;
  });
  return keyedResults;
}
  ...
  function errorHandler(error, routeId, nextResult) {
    if (nextResult) {
      ...
      let maxBoundaryIdx = Math.min(
        // Throwing route
        Math.max(
          matches.findIndex((m) => m.route._id === routeId),
          0
        ),
      ...
      let boundaryRouteId = findNearestBoundary(
        matches,
        matches[maxBoundaryIdx].route._id
      ).route._id;
      return Promise.resolve({
...
async function runMiddlewarePipeline(args, handler, processResult, isResult, errorHandler) {
  ...
  let tuples = matches.flatMap(
    (m) => m.route.middleware ? m.route.middleware.map((fn) => [m.route._id, fn]) : []
  );
...
function getTargetedDataStrategyMatches(mapRouteProperties2, manifest, request, matches, targetMatch, lazyRoutePropertiesToSkip, scopedContext, shouldRevalidateArgs = null) {
  return matches.map((match) => {
    if (match.route._id !== targetMatch.route._id) {
      return {
        ...match,
        shouldLoad: false,
    ...
    let actualHandler = (ctx) => {
      if (typeof handler !== "function") {
        return Promise.reject(
          new Error(
            `You cannot call the handler for a route which defines a boolean "${type}" [routeId: ${match.route._id}]`
          )
        );
      }
      return handler(
...
function normalizeRelativeRoutingRedirectResponse(response, request, routeId, matches, basename) {
  ...
  if (!isAbsoluteUrl(location2)) {
    let trimmedMatches = matches.slice(
      0,
      matches.findIndex((m) => m.route._id === routeId) + 1
    );
  ...
  matches.forEach((match) => {
    ...
    if (isErrorResult(result)) {
      ...
      if (skipLoaderErrorBubbling) {
        ...
        if (errors[boundaryMatch.route._id] == null) {
          errors[boundaryMatch.route._id] = error;
        }
      }
      if (!isStaticHandler) {
...
function processLoaderData(state, matches, results, pendingActionResult, revalidatingFetchers, fetcherResults) {
  ...
  revalidatingFetchers.filter((f) => !f.matches || f.matches.some((m) => m.shouldLoad)).forEach((rf) => {
    ...
    if (isErrorResult(result)) {
      let boundaryMatch = findNearestBoundary(state.matches, match?.route._id);
      if (!(errors && errors[boundaryMatch.route._id])) {
        errors = {
          ...errors,
          [boundaryMatch.route._id]: result.error
        };
...
function findNearestBoundary(matches, routeId) {
  let eligibleMatches = routeId ? matches.slice(0, matches.findIndex((m) => m.route._id === routeId) + 1) : [...matches];
...
function RenderedRoute({ routeContext, match, children }) {
  ...
  if (dataRouterContext && dataRouterContext.static && dataRouterContext.staticContext && (match.route.errorElement || match.route.ErrorBoundary)) {
    dataRouterContext.staticContext._deepestRenderedBoundaryId = match.route._id;
  }
...
function _renderMatches(matches, parentMatches = [], dataRouterState = null, unstable_onError = null, future = null) {
  ...
  if (errors != null) {
    let errorIndex = renderedMatches.findIndex(
      (m) => m.route._id && errors?.[m.route._id] !== void 0
    );
    invariant(
      errorIndex >= 0,
  ...
  if (dataRouterState) {
    for (let i = 0; i < renderedMatches.length; i++) {
      ...
      if (match.route._id) {
        ...
        let needsToRunLoader = match.route.loader && !loaderData.hasOwnProperty(match.route._id) && (!errors2 || errors2[match.route._id] === void 0);
  ...
  return renderedMatches.reduceRight(
    (outlet, match, index) => {
      ...
      if (dataRouterState) {
        error = errors && match.route._id ? errors[match.route._id] : void 0;
...
function createRoutesFromChildren(children, parentPath = []) {
  ...
  React3.Children.forEach(children, (element, index) => {
    ...
    let route = {
      id: element.props._id || treePath.join("-"),
...
function getTurboStreamSingleFetchDataStrategy(getRouter, manifest, routeModules, ssr, basename) {
  let dataStrategy = getSingleFetchDataStrategyImpl(
    getRouter,
    (match) => {
      let manifestRoute = manifest.routes[match.route._id];
      invariant2(manifestRoute, "Route not found in manifest");
      let routeModule = routeModules[match.route._id];
      return {
...
async function singleFetchActionStrategy(args, fetchAndDecode, basename) {
  ...
  let result = await actionMatch.resolve(async (handler) => {
    let result2 = await handler(async () => {
      ...
      actionStatus = status;
      return unwrapSingleFetchResult(data2, actionMatch.route._id);
    });
    return result2;
  });
  if (isResponse(result.result) || isRouteErrorResponse(result.result) || isDataWithResponseInit(result.result)) {
    return { [actionMatch.route._id]: result };
  }
  return {
    [actionMatch.route._id]: {
      type: result.type,
    ...
    matchesToLoad.map(
      (m) => m.resolve(async (handler) => {
        try {
          ...
          let routeId = m.route._id;
          let result = hasClientLoader ? await handler(async () => {
            let { data: data2 } = await fetchAndDecode(args, basename, [routeId]);
            return unwrapSingleFetchResult(data2, routeId);
          }) : await handler();
          results[m.route._id] = { type: "data", result };
        } catch (e) {
          results[m.route._id] = { type: "error", result: e };
        }
      })
    )
  );
...
async function bubbleMiddlewareErrors(singleFetchPromise, matches, routesParams, results) {
  try {
    ...
    if ("routes" in fetchedData) {
      for (let match of matches) {
        if (match.route._id in fetchedData.routes) {
          let routeResult = fetchedData.routes[match.route._id];
          if ("error" in routeResult) {
            middlewareError = routeResult.error;
            if (results[match.route._id]?.result == null) {
              results[match.route._id] = {
                type: "error",
...
async function singleFetchLoaderFetcherStrategy(args, fetchAndDecode, basename) {
  ...
  let routeId = fetcherMatch.route._id;
  ...
  return { [fetcherMatch.route._id]: result };
}
...
async function loadRouteModule(route, routeModulesCache) {
  if (route._id in routeModulesCache) {
    return routeModulesCache[route._id];
  }
  try {
    ...
    routeModulesCache[route._id] = routeModule;
    return routeModule;
  } catch (error) {
    console.error(
...
function getKeyedLinksForMatches(matches, routeModules, manifest) {
  let descriptors = matches.map((match) => {
    let module = routeModules[match.route._id];
    let route = manifest.routes[match.route._id];
    return [
...
function getNewMatchesForLinks(page, nextMatches, currentMatches, manifest, location2, mode) {
  let isNew = (match, index) => {
    ...
    return match.route._id !== currentMatches[index].route._id;
  };
...
function getRouteComponents(route, routeModule, isSpaMode) {
  ...
  let HydrateFallback = routeModule.HydrateFallback && (!isSpaMode || route._id === "root") ? routeModule.HydrateFallback : route._id === "root" ? RemixRootDefaultHydrateFallback : void 0;
  let ErrorBoundary = routeModule.ErrorBoundary ? routeModule.ErrorBoundary : route._id === "root" ? () => React6.createElement(RemixRootDefaultErrorBoundary, { error: useRouteError() }) : void 0;
  if (route._id === "root" && routeModule.Layout) {
    return {
      ...Component4 ? {
...
function createServerRoutes(manifest, routeModules, future, isSpaMode, parentId = "", routesByParentId = groupRoutesByParentId(manifest), spaModeLazyPromise = Promise.resolve({ Component: () => null })) {
  return (routesByParentId[parentId] || []).map((route) => {
    let routeModule = routeModules[route._id];
    invariant2(
      routeModule,
...
function preventInvalidServerHandlerCall(type, route) {
  if (type === "loader" && !route.hasLoader || type === "action" && !route.hasAction) {
    ...
    let msg = `You are trying to call ${fn} on a route that does not have a server ${type} (routeId: "${route._id}")`;
    console.error(msg);
...
function createClientRoutes(manifest, routeModulesCache, initialState, ssr, isSpaMode, parentId = "", routesByParentId = groupRoutesByParentId(manifest), needsRevalidation) {
  return (routesByParentId[parentId] || []).map((route) => {
    let routeModule = routeModulesCache[route._id];
    ...
    function fetchServerAction(singleFetch) {
      if (!route.hasAction) {
        throw noActionDefinedError("action", route._id);
      }
      return fetchServerHandler(singleFetch);
    }
    ...
    async function prefetchStylesAndCallHandler(handler) {
      let cachedModule = routeModulesCache[route._id];
    ...
    if (routeModule) {
      ...
      let hasInitialData = initialState && initialState.loaderData && route._id in initialState.loaderData;
      let initialData = hasInitialData ? initialState?.loaderData?.[route._id] : void 0;
      let hasInitialError = initialState && initialState.errors && route._id in initialState.errors;
      let initialError = hasInitialError ? initialState?.errors?.[route._id] : void 0;
      ...
      dataRoute.action = ({ request, params, context }, singleFetch) => {
        return prefetchStylesAndCallHandler(async () => {
          ...
          if (!routeModule.clientAction) {
            if (isSpaMode) {
              throw noActionDefinedError("clientAction", route._id);
            }
...
function getPartialManifest({ sri, ...manifest }, router2) {
  let routeIds = new Set(router2.state.matches.map((m) => m.route._id));
  ...
  paths.forEach((path) => {
    ...
    if (matches) {
      matches.forEach((m) => routeIds.add(m.route._id));
    }
  });
...
async function fetchAndApplyManifestPatches(paths, errorReloadPath, manifest, routeModules, ssr, isSpaMode, basename, manifestPath, patchRoutes, signal) {
  ...
  let patches = Object.values(serverPatches).reduce((acc, route) => {
    if (route && !knownRoutes.has(route._id)) {
      acc[route._id] = route;
    }
    return acc;
  }, {});
...
function getActiveMatches(matches, errors, isSpaMode) {
  ...
  if (errors) {
    let errorIdx = matches.findIndex((m) => errors[m.route._id] !== void 0);
...
function PrefetchPageLinksImpl({
  ...
  let dataHrefs = React8.useMemo(() => {
    ...
    nextMatches.forEach((m) => {
      let manifestRoute = manifest.routes[m.route._id];
      if (!manifestRoute || !manifestRoute.hasLoader) {
        return;
      }
      if (!newMatchesForData.some((m2) => m2.route._id === m.route._id) && m.route._id in loaderData && routeModules[m.route._id]?.shouldRevalidate) {
        foundOptOutRoute = true;
      } else if (manifestRoute.hasClientLoader) {
        foundOptOutRoute = true;
      } else {
        routesParams.add(m.route._id);
      }
    });
    ...
    if (foundOptOutRoute && routesParams.size > 0) {
      url.searchParams.set(
        "_routes",
        nextMatches.filter((m) => routesParams.has(m.route._id)).map((m) => m.route._id).join(",")
      );
    }
...
function Meta() {
  ...
  let error = null;
  if (errors) {
    error = errors[_matches[_matches.length - 1].route._id];
  }
  let meta = [];
  let leafMeta = null;
  let matches = [];
...
${matches.map((match, routeIndex) => {
      ...
      let manifestEntry = manifest.routes[match.route._id];
      invariant2(manifestEntry, `Route ${match.route._id} not found in manifest`);
      let {
        clientActionModule,
        clientLoaderModule,
  ...
  window.__reactRouterRouteModules = {${matches.map((match, index) => `${JSON.stringify(match.route._id)}:route${index}`).join(",")}};
...
function useFetcher({
  ...
  let routeId = route.matches[route.matches.length - 1]?.route._id;
...
function createStaticRouter(routes, context, opts = {}) {
  ...
  let matches = context.matches.map((match) => {
    let route = manifest[match.route._id] || match.route;
    return {
      ...match,
      route
    };
...
function processRoutes(routes, context, manifest, routeModules, parentId) {
  return routes.map((route) => {
    if (!route._id) {
      throw new Error(
        "Expected a route._id in react-router processRoutes() function"
      );
    }
    let newRoute = {
      id: route._id,
    ...
    manifest.routes[newRoute._id] = entryRoute;
    routeModules[route._id] = {
...
function createRoutes(manifest, parentId = "", routesByParentId = groupRoutesByParentId2(manifest)) {
  return (routesByParentId[parentId] || []).map((route) => ({
    ...route,
    children: createRoutes(manifest, route._id, routesByParentId)
  }));
}
function createStaticHandlerDataRoutes(manifest, future, parentId = "", routesByParentId = groupRoutesByParentId2(manifest)) {
  return (routesByParentId[parentId] || []).map((route) => {
    let commonRoute = {
      ...
      hasErrorBoundary: route._id === "root" || route.module.ErrorBoundary != null,
      id: route._id,
      path: route.path,
      ...
      loader: route.module.loader ? async (args) => {
        ...
        if (preRenderedData != null) {
          ...
          if (data2 && SingleFetchRedirectSymbol in data2) {
            ...
            invariant22(
              data2 && route._id in data2,
              "Unable to decode prerendered data"
            );
            let result = data2[route._id];
            invariant22(
              "data" in result,
...
function getDocumentHeaders(context, build) {
  return getDocumentHeadersImpl(context, (m) => {
    let route = build.routes[m.route._id];
    invariant22(route, `Route with id "${m.route._id}" not found in build`);
    return route.module.headers;
  });
}
function getDocumentHeadersImpl(context, getRouteHeadersFn, _defaultHeaders) {
  let boundaryIdx = context.errors ? context.matches.findIndex((m) => context.errors[m.route._id]) : -1;
...
async function singleFetchLoaders(build, serverMode, staticHandler, request, handlerUrl, loadContext, handleError) {
  ...
  try {
    ...
    let result = await staticHandler.query(handlerRequest, {
      requestContext: loadContext,
      filterMatchesToLoad: (m) => !loadRouteIds || loadRouteIds.has(m.route._id),
      skipLoaderErrorBubbling: true,
      generateMiddlewareResponse: build.future.v8_middleware ? async (query) => {
        try {
          let innerResult = await query(handlerRequest);
  ...
  function staticContextToResponse(context) {
    ...
    let loadedMatches = new Set(
      context.matches.filter(
        (m) => loadRouteIds ? loadRouteIds.has(m.route._id) : m.route.loader != null
      ).map((m) => m.route._id)
    );
    if (context.errors) {
...
var isSession = (object) => {
  return object != null && typeof object._id === "string" && typeof object.data !== "undefined" && typeof object.has === "function" && typeof object.get === "function" && typeof object.set === "function" && typeof object.flash === "function" && typeof object.unset === "function";
};
...
function createRouterFromPayload({
  ...
  let routes = payload.matches.reduceRight((previous, match) => {
    ...
    if (previous.length > 0) {
      ...
      let childrenToPatch = patches.get(match._id);
      if (childrenToPatch) {
        route.children.push(
  ...
  globalVar.__reactRouterDataRouter = createRouter({
    ...
    hydrationData: getHydrationData({
      ...
      getRouteInfo: (routeId) => {
        let match = payload.matches.find((m) => m._id === routeId);
  ...
  globalVar.__reactRouterDataRouter._updateRoutesForHMR = (routeUpdateByRouteId) => {
    ...
    function walkRoutes(routes2, parentId) {
      return routes2.map((route) => {
        const routeUpdate = routeUpdateByRouteId.get(route._id);
        if (routeUpdate) {
          const {
            routeModule,
            hasAction,
          ...
          if (route.children) {
            newRoute.children = walkRoutes(route.children, route._id);
          }
          return newRoute;
        }
        const updatedRoute = { ...route };
        if (route.children) {
          updatedRoute.children = walkRoutes(route.children, route._id);
        }
        return updatedRoute;
      });
    }
...
function getRSCSingleFetchDataStrategy(getRouter, ssr, basename, createFromReadableStream, fetchImplementation) {
  ...
  return async (args) => args.runClientMiddleware(async () => {
    ...
    for (const route of context.get(renderedRoutesContext)) {
      if (!renderedRoutesById.has(route._id)) {
        renderedRoutesById.set(route._id, []);
      }
      renderedRoutesById.get(route._id).push(route);
    }
    for (const match of args.matches) {
      const renderedRoutes = renderedRoutesById.get(match.route._id);
      if (renderedRoutes) {
...
function createRouteFromServerManifest(match, payload) {
  let hasInitialData = payload && match._id in payload.loaderData;
  let initialData = payload?.loaderData[match._id];
  let hasInitialError = payload?.errors && match._id in payload.errors;
  let initialError = payload?.errors?.[match._id];
  ...
  let dataRoute = {
    ...
    action: match.clientAction ? (args, singleFetch) => match.clientAction({
      ...
      throw noActionDefinedError("action", match._id);
    },
    path: match.path,
...
function RSCStaticRouter({ getPayload }) {
  ...
  for (const match of payload.matches) {
    if (shouldHydrateRouteLoader(
      ...
      delete patchedLoaderData[match._id];
    }
  }
  const context = {

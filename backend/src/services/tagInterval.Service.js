...
export const getTagsOfInterest = async ({userId, start, end}) => {
    const { tagTotals, numberOfTasks } = await totalTimeActiveForEachTag({userId, startTime: start, endTime: end});
    const tagIds = Object.keys(tagTotals);

    const tags = await getTagsCachedByMultipleIds(userId, [...tagIds]);
